...
function placeAll(){
  ...
  for(const sec of Object.keys(BEARING)){const n=ITEMS[sec].length;if(!n)continue;out[sec]=[];
    ...
    for(let i=0;i<n;i++){const it=ITEMS[sec][i];const H=estH(it);
      ...
      let p,box,ok=false,tries=0,w=1;
      while(!ok&&tries<400){const a=BEARING[sec]+gauss()*sigA*w,r=base+gauss()*40*w;tries++;
        p=[CX+Math.cos(a)*r-NODE_W/2,CY+Math.sin(a)*r-H/2];
        p[0]=Math.min(FW-NODE_W-40,Math.max(40,p[0]));p[1]=Math.min(FH-H-40,Math.max(40,p[1]));
